...
CSV_ENCODINGS = [
    {'id': 'utf-8-sig', 'label': 'UTF-8 с BOM (рекомендуется)',
     'hint': 'Excel и большинство CRM распознают кодировку автоматически.'},
    {'id': 'utf-8', 'label': 'UTF-8 без BOM',
     'hint': 'Если импортёр спотыкается о BOM в первой колонке заголовка.'},
    {'id': 'cp1252', 'label': 'Windows-1252 (латиница)',
     'hint': 'Если система жёстко ждёт однобайтовую кодировку. '
             'Испанские é, í, ñ сохраняются, кириллица — нет.'},
    {'id': 'cp1251', 'label': 'Windows-1251 (кириллица)',
     'hint': 'Только если в данных есть русский текст и импортёр ждёт ANSI.'},
]
...
def read_xlsx(path):
    ...
    if (ws.max_row or 0) > MAX_ROWS:
        raise TableTooLargeError(f'Слишком много строк: {ws.max_row} (лимит {MAX_ROWS}).')
    headers = []
...
def read_csv_bytes(raw_bytes):
    ...
    if len(data) > MAX_ROWS:
        raise TableTooLargeError(f'Слишком много строк: {len(data)} (лимит {MAX_ROWS}).')
    ...
    for m in members:
        for k in record_dup_keys(m, opts):
            if k in seen and seen[k] is not m:
                reasons.add({'n': 'название', 'e': 'e-mail', 'p': 'телефон', 'w': 'сайт'}[k[0]])
            seen[k] = m
    return reasons
...
_ACTION_RU = {'moved': 'перенесено', 'duplicate': 'дубликат, отброшено'}
_KIND_RU = {'email': 'e-mail', 'phone': 'телефон', 'url': 'сайт'}
